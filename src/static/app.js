document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
        ...
        if (details.participants.length > 0) {
          participantsHTML = details.participants.map(email => `
            <li class="participant-item">
              ...
              <span class="delete-participant" title="Remover" data-activity="${name}" data-email="${email}">&#128465;</span>
            </li>
          `).join('');
        } else {
      ...
      // Adiciona event listener para ícones de exclusão
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        signupForm.reset();
        await fetchActivities(); // Aguarda atualização da lista antes de mostrar mensagem
        messageDiv.textContent = result.message;
        messageDiv.className = "success";
      } else {
